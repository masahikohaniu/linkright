...
CPubKey CWallet::GenerateVanityKey(CWalletDB &walletdb,const std::string& strPattern)
{
    AssertLockHeld(cs_wallet); // mapKeyMetadata
    bool fCompressed = CanSupportFeature(FEATURE_COMPRPUBKEY); // default to compressed public keys if we want 0.6.0 wallets
    ...
    while (!bfound) {
        ...
        assert(secret.VerifyPubKey(pubkey));
        strPubkey = EncodeDestination(pubkey.GetID());
	int length = strPattern.length();
	//strFound=boost::algorithm::to_lower_copy(strPubkey.substr(1,length)); //case insensitive
        strFound=strPubkey.substr(1,length);   //case sensitive 
	if (strPattern.compare(strFound) == 0){
	    bfound =true;
	    break;
	} else if (strPattern.substr(0,length-1).compare(strFound.substr(0,length-1)) == 0){
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-linkright-address
...
bool CWallet::VanityKey(const std::string& strPattern)
{
    {
        LOCK(cs_wallet);
        if (IsLocked())
        ...
        CWalletDB walletdb(*dbw);
	int64_t index = ++m_max_keypool_index;
	CPubKey pubkey(GenerateVanityKey(walletdb,strPattern));
	if (!walletdb.WritePool(index, CKeyPool(pubkey, false))) {
	    throw std::runtime_error(std::string(__func__) + ": writing generated key failed");
	}
